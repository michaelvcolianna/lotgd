...
This is the Dragonprime 1.1.2 code that _should_ run on shared hosting. Note
that if your server is using PHP >= 7, you'll need to modify the
`/lib/dbwrapper.php` file to use something other than 'mysql' or the database
won't work.

The [old readme](#old-readme) is below for archival purposes. Some slightly
updated code (which _should not_ work on shared hosting) is in the `develop`
branch.
